...
class AppDelegate: NSObject, NSApplicationDelegate, AudioInputDelegate {
    ...
    private var lastDetectedSampleRate: Int = 0
    ...
    private func handleSampleRateChange(_ rate: Int) {
        ...
        // 0.1 檢查是否與上次偵測到的採樣率相同
        if rate == lastDetectedSampleRate {
            print("ℹ️ Sample rate \(rate) Hz is same as current. Skipping switch.")
            return
        }
